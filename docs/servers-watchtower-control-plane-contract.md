...
- Environment names begin an API-owned `pending` registration on first valid collection through the Ingest admission contract. A pending registration reserves one slot against the 1,000-registration project limit but is not active or listable; it becomes active only with the matching durable Ingest acceptance. Failed admission rolls it back idempotently, while an uncertain post-acceptance activation remains pending and retryable until reconciled. Manage may hide active registrations from the normal list; renaming, environment data deletion, and environment-level authorization are unsupported. Hidden active registrations still count toward limits. Owner/Admin may retire a registration to reclaim its slot without deleting historical data, and explicitly permit that name again through the registration lifecycle below.
...
- Reserve one of a separate per-user ten-slot external-allocation cleanup budget before provisioning a provisional WorkOS organization. Outstanding provisional allocations reserve their possible cleanup slot, so cancellation can always convert that reservation to cleanup-pending without exceeding ten. Cancellation releases the ownership reservation immediately but holds this separate slot until API confirms external deletion/nonexistence; successful activation releases the cleanup reservation. Unknown provider outcomes remain charged until reconciled. At this bound, reject new provisioning with 429 while permitting cancellation, cleanup and recovery of existing operations. Counters and cleanup intents are durable and serialized; retries and account deletion cannot release an unconfirmed slot, lose cleanup ownership or allocate under a new operation to evade the bound. Follow existing account-context erasure deadlines for identifying cleanup records.
...
| Authorize external-membership reconciliation | Different current Owner with recent user-session authentication; otherwise affected Owner recovery evidence and separate Support approval |
...
- API keeps authoritative restore-independent recovery fences for both personal and organization/Owner bundles: monotonically increasing bundle/contact generations, invalidation cutoffs, and single-code consumption claims bound to the exact recovery request. Persist an atomic conditional consumption claim before accepting the proof; competing requests cannot consume the same code. An identical retry resumes its original request and never grants a second recovery capability. A crash after the claim reconciles that request rather than making the code available again.
- Bundle/contact replacement, Owner loss, MFA reset and identity reconnection durably record their generation transition/invalidation intent before acknowledging completion. Reconcile pending transitions to the same confirmed replacement generation under existing atomic activation rules; a restore cannot activate old bundles or invalidate a completed replacement again. Store only opaque scope/code identifiers and versions in immutable append-only fence records, with identifying context independently erasable; never store plaintext codes or recoverable secrets in the fence journal. Retain fences through all affected backup horizons and unresolved requests. API reconciles the latest fences before checking any recovery hash or exposing recovery access; missing, regressed or unverifiable records fail closed. Audit evidence alone is not the authority for consumption.
- Test concurrent code consumption and crashes before/after consumption claims, contact/bundle replacement and invalidation. Restore an older API database and retry the original and a different request: only the original pending request may resume, and no consumed code, obsolete bundle or revoked recovery capability becomes usable.
...
- Advance the fetched cursor only in the same durable commit as an idempotent inbox record for every event it passes. A crash before commit refetches that range; a crash afterward replays unapplied inbox records. Never persist a cursor independently of the events it skips. Track fetched and fully applied positions separately: successful fetches alone do not confirm security synchronization. Advance the applied position only after all preceding events have durable authoritative outcomes, including required revocation intents and acknowledged fences. Pending security events cannot be hidden by later successful polls; the existing five-minute bound is measured from fully applied confirmation.
- Reconcile the cursor, inbox and restore-independent security intents before readiness. Retain unapplied events until durable handling; deduplication resumes incomplete processing rather than treating receipt as completion. Missing history or unverifiable cursor/inbox alignment blocks affected user access until an authoritative resynchronization completes; never skip forward to the newest event. Test crashes around both commits, duplicate/redelivered events, delayed revocation acknowledgements and an old-backup restore while newer polls succeed.
...
- API owns a restricted external-membership reconciliation operation for an intact external user and existing Watchtower membership. A different human who is a current Owner of that organization may authorize it using a user session reauthenticated within five minutes and satisfying the organization's policy. If no other Owner can authorize recovery, an affected current Owner must use the existing registered recovery email, organization code and separate Support approval, with existing expiry and human-separation rules. Neither Admin nor a token nor a provider callback can clear the block independently.
- Bind reconciliation to the exact Watchtower account/organization, current external user and organization binding, blocked-membership generation and reviewed current role/grants. API repairs or reactivates the external membership and verifies its current state before clearing the local block. Recheck approver authority, target membership, policy, suspension/deletion state and expected versions at commit. Removed Watchtower membership requires a fresh invitation instead; repair cannot add roles, restore withdrawn grants, reconnect a deleted identity or change the external organization binding. Authentication and MFA/SSO conditions must be met before ordinary access resumes, and previously revoked credentials remain revoked.
- Persist the blocking intent and reconciliation generation in the restore-independent security registry and apply existing owner-acknowledged fences. Only an approved, durably completed generation clears this organization's block; other organizations remain unchanged. Record the external membership identity and verified event-processing boundary at completion so known older/duplicate events cannot overwrite the result. An event not provably older triggers a fresh provider-state check and keeps access blocked while ambiguous; never discard a genuine later deletion. Provider repair without Watchtower approval does not clear the block. Failed/partial repair remains pending, idempotent retries resume the same operation, and stale approvals cannot clear a newer block. Reconcile generations before readiness after restore.
- Audit requests and results and notify the affected user and organization Owners through existing durable notification intent. Test accidental external deletion/deactivation, distinct-Owner approval and sole-Owner recovery, lost approver authority, removed grants, policy changes, repair response loss, stale/duplicate/later deletion events, suspension and old-backup restore. No case may revive revoked credentials or grant access to another organization.
...
- Account deletion atomically terminalizes all organization-creation preparations initiated by the account and all unfinished Owner promotions targeting it, releases their ownership reservations exactly once, and fences late verification/provisioning callbacks. Do not require the deleted account to log in to cancel them. Persist these cancellations with the restore-independent deletion intent before acknowledging deletion; activation and promotion serialize with account state so a race cannot create a deleted Owner. Schedule provisional external-organization cleanup and erase preparation email/code payload under the account's original 14/90-day deadlines. Cleanup retries retain only required erasable operational evidence and cannot reopen preparation.
...
- Account and organization deletion require a client-prepared cryptographically random 256-bit receipt secret, retained by the client before submitting the destructive request. Bind its verification hash to the exact principal, deletion scope, operation and idempotency payload before committing any revocation. Store only that hash; never return or log the secret. A lost response therefore does not lose the ability to authenticate the already-submitted operation's status. Idempotent retries cannot replace its hash or scope.
- A separate API-owned deletion-receipt status operation accepts the secret even after account/session or organization authority revocation. This is a narrow authentication exception for the bound operation's minimal pending/completed/failed result and operation identifier only: no resource contents, names, audit history, general access, retry execution, cancellation or restoration. Invalid receipts do not disclose whether the target or operation exists. An ordinary deletion request still requires current authentication and authorization; after revocation the client uses status lookup rather than replaying an unauthorized command.
- The receipt is valid during unfinished execution and until 24 hours after termination, then its verification material is erased. Persist its binding with restore-independent deletion operation state so crashes cannot lose the result or revive an expired receipt; no identifying profile is needed. Apply the existing independent security budget using the bound opaque principal/organization scope, plus the preauthentication IP limit before receipt verification. Unavailable enforcement or operation evidence returns 503; no unrestricted lookup fallback. Keep receipt secrets out of URLs, telemetry and notification messages.
- Test response loss before/after deletion commit, revoked sessions, organization membership removal, valid receipt lookup and secret guessing, mismatched scopes/payloads, expiry at 24 hours after termination, long-running deletion, audit failure and backup restore. The receipt must neither authorize another operation nor reveal deleted data.
...
| --- | ---: |
| Provisional/cleanup-pending external allocations per user | 10 |
...
- At the environment limit, reject a new pending registration while continuing collection for existing environments. Pending reservations remain charged until activation or confirmed rollback, so concurrent first collections cannot exceed the limit.
- API owns versioned environment registration, retirement, and reactivation. A first collection creates or resumes a pending registration keyed by project/name/generation and the Ingest admission identity; durable acceptance activates it, while a rejected admission group or confirmed uncertainty rollback releases the reserved slot exactly once. Retirement writes a durable name tombstone and obtains Ingest's admission-fence acknowledgement before releasing the active slot; collection for that name is then rejected, never silently dropped or automatically re-registered. Accepted data finishes normally and retained historical reads remain authorized. Explicit reactivation atomically checks/reserves capacity and advances the registration generation before admitting that name again. Retirement is idempotent and cannot release a slot twice; concurrent first collection, admission failure, activation retry, retirement, and reactivation serialize on project/name and quota state. Tombstones, pending-operation state, and generation fences survive backup restore and stay effective until an authorized newer reactivation or project deletion. Repeated retirement of already-retired names creates no extra registration. No new environment ACL is introduced.
- Test quota exhaustion from a compromised DSN, failed primary/attachment/auxiliary admission after pending registration, activation response loss, retirement with in-flight collection, blocked automatic re-registration, explicit at-cap reactivation, hidden registrations, stale projection replay and backup restore. Retiring a name must preserve its historical data and free exactly one slot only after admission fencing.
...
- Emergency requester and approver must be different humans, established by API's verified stable staff-person records and linked current/previous registrations. Different staff IDs, WorkOS identities or email addresses do not prove separation. Persist the verified linkage with the approval and recheck it and both current Operator assignments at approval, capability issuance and each access. Missing/ambiguous linkage blocks access; neither participant may alter their own linkage to qualify. Reconnection or role changes cannot bypass this check.
...
| Deletion receipt status | API; deleting client through restricted receipt authentication | Exact immutable deletion operation, minimal status only, bounded lifetime/rates and restore-independent state; no ordinary access grant |
| Customer audit-history reads | Query public reads to API through authenticated internal unary handoff | Current Owner/Admin and credential scope, bounded scope-bound pagination, live erasable context; no Query persistence/cache or stale fallback |
...
| External membership reconciliation | Blocked existing membership to approved provider repair, then version-checked local unblock | Current permissions and authentication only; newer blocks, suspension and deletion prevail; other organizations remain blocked |
...
- Keep WorkOS cleanup unavailable while repeatedly preparing and canceling organizations; verify provisional plus cleanup-pending allocations never exceed ten, ownership slots release independently, uncertain outcomes retain their charge, and confirmed cleanup releases exactly one slot across crashes and account deletion.
...
- Delete an account during creation or promotion preparation, including concurrent activation and late email/provider callbacks. Verify terminal cancellation, exactly-once reservation release, external cleanup retries, original erasure deadlines and backup-restore fencing; no deleted Owner may be created.
...
- Request and approve emergency access through two registrations or replacement identities of the same person; reject self-approval and missing/ambiguous person linkage before capability issuance and each read. Verify revoked Operator authority cancels pending and active access.
